...
#include "Setting.h"
#include <windows.h>
#include <VersionHelpers.h> // Ensure this header is included for IsWindowsVersionOrGreater
...
{
	// Use VersionHelpers.h macro instead of deprecated GetVersionEx
	// Fix: Remove recursion and use IsWindowsVersionOrGreater for NT detection
	return IsWindowsVersionOrGreater(5, 0, 0); // Windows 2000 (NT) or later
}
...
{
	CFileDialog dlg(FALSE, ".cue", nullptr, OFN_HIDEREADONLY, MSG(125));
	UpdateData(TRUE);
