...
void CTocWindow::OnFileSavetxt()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		for (i = 0; i < m_View.GetItemCount(); i++)
		{
			...
			fprintf(fp, "%02d %5s %8s %8s %8s %13s %4s %s\n", i, (LPCTSTR)TrackType, (LPCTSTR)Start, (LPCTSTR)Length, (LPCTSTR)Gap, (LPCTSTR)Size, (LPCTSTR)DigitalCopy,
				(LPCTSTR)Emphasis);
		}
...
void CTocWindow::OnFileSavehtml()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		cs = m_StatusBar.GetStatusBarCtrl().GetText(1);
		fprintf(fp, "<tr><td colspan=\"4\" align=\"center\">%s</td>", (LPCTSTR)cs);
		cs = m_StatusBar.GetStatusBarCtrl().GetText(2);
		...
		for (i = 0; i < m_View.GetItemCount(); i++)
		{
			...
			fprintf(
				...
				i, (LPCTSTR)TrackType, (LPCTSTR)Start, (LPCTSTR)Length, (LPCTSTR)Gap, (LPCTSTR)Size, (LPCTSTR)DigitalCopy, (LPCTSTR)Emphasis);
		}
...
void CTocWindow::OnFileSavecsv()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		for (i = 0; i < m_View.GetItemCount(); i++)
		{
			...
			DigitalCopy = m_View.GetItemText(i, 6);
			Emphasis = m_View.GetItemText(i, 7);
			fprintf(fp, "\"%d\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"\n", i, (LPCTSTR)TrackType, (LPCTSTR)Start, (LPCTSTR)Length, (LPCTSTR)Gap,
				(LPCTSTR)Size, (LPCTSTR)DigitalCopy, (LPCTSTR)Emphasis);
		}
