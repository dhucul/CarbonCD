...
bool CCueSheetParser::Parse(LPCSTR CueSheet, LPCSTR ImageDir, DWORD ImageSize)
{
    ...
    while (AbstractCueLine())
    {
        if (StringCompare(m_CueLine, "TRACK", 5))
        {
            ...
            int TrackNo;
            int TrackType;
            p = static_cast<LPCSTR>(m_CueLine) + 5;
            ...
            else if (StringCompare(p, "MODE1/2352", 10))
            {
                TrackType = TRACK_MODE1_RAW;
                m_DataMode = 1;
            }

            else if (StringCompare(p, "MODE2/2352", 10))
            {
                TrackType = TRACK_MODE1_RAW;
                m_DataMode = 2;
            }
        ...
        else if (StringCompare(m_CueLine, "INDEX", 5))
        {
            LPCSTR p;
            int IndexNo, m, s, f;
            p = static_cast<LPCSTR>(m_CueLine) + 5;
        ...
        else if (StringCompare(m_CueLine, "FILE", 4))
        {
            LPCSTR p;
            CString ImageFile;
            p = static_cast<LPCSTR>(m_CueLine) + 4;
            ...
            {
                HANDLE hFile;
                hFile = CreateFile(m_ImageFile, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
                ...
                m_ImageSize = GetFileSize(hFile, nullptr);
                CloseHandle(hFile);
            }
        }
...
int CCueSheetParser::CreateMMCCueSheet(DWORD LeadInLBA)
{
    ...
    m_MMCCue[Entry + 5] = ToHex(static_cast<BYTE>(LeadInLBA / (60 * 75))); //   Absolute Minute
    m_MMCCue[Entry + 6] = ToHex(static_cast<BYTE>((LeadInLBA / 75) % 60)); //   Absolute Second
    m_MMCCue[Entry + 7] = ToHex(static_cast<BYTE>(LeadInLBA % 75)); //   Absolute Frame
    ...
    for (i = 0; i < m_TrackCount; i++)
    {
        if (m_Cue[i].m_PregapLBA != 0xffffffff)
        {
            ...
            m_MMCCue[Entry + 3] = 0x00; //   Data Form
            m_MMCCue[Entry + 4] = 0x00; //   Serial Copy Management System
            m_MMCCue[Entry + 5] = ToHex(static_cast<BYTE>((m_Cue[i].m_PregapLBA + 150) / (60 * 75)));
            //   Absolute Minute
            m_MMCCue[Entry + 6] = ToHex(static_cast<BYTE>(((m_Cue[i].m_PregapLBA + 150) / 75) % 60));
            //   Absolute Second
            m_MMCCue[Entry + 7] = ToHex(static_cast<BYTE>((m_Cue[i].m_PregapLBA + 150) % 75)); //   Absolute Frame
        ...
        m_MMCCue[Entry + 3] = 0x00; //   Data Form
        m_MMCCue[Entry + 4] = 0x00; //   Serial Copy Management System
        m_MMCCue[Entry + 5] = ToHex(static_cast<BYTE>((m_Cue[i].m_StartLBA + 150) / (60 * 75))); //   Absolute Minute
        m_MMCCue[Entry + 6] = ToHex(static_cast<BYTE>(((m_Cue[i].m_StartLBA + 150) / 75) % 60)); //   Absolute Second
        m_MMCCue[Entry + 7] = ToHex(static_cast<BYTE>((m_Cue[i].m_StartLBA + 150) % 75)); //   Absolute Frame
    ...
    m_MMCCue[Entry + 3] = 0x01; //   Data Form
    m_MMCCue[Entry + 4] = 0x00; //   Serial Copy Management System
    m_MMCCue[Entry + 5] = ToHex(static_cast<BYTE>((m_Cue[m_TrackCount].m_StartLBA + 150) / (60 * 75)));
    //   Absolute Minute
    m_MMCCue[Entry + 6] = ToHex(static_cast<BYTE>(((m_Cue[m_TrackCount].m_StartLBA + 150) / 75) % 60));
    //   Absolute Second
    m_MMCCue[Entry + 7] = ToHex(static_cast<BYTE>((m_Cue[m_TrackCount].m_StartLBA + 150) % 75)); //   Absolute Frame
    Entry += 8;
