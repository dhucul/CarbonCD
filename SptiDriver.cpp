...
bool CSptiDriver::IsDriveReady()
{
	...
	memcpy(sptd.Cdb, cdb, sizeof(cdb));
...
void CSptiDriver::GetDiscInformation(CString& discInfo)
{
	...
	cmd.SRB_Flags = SRB_DIR_IN;
	cmd.SRB_CDBLen = 10;
	cmd.CDBByte[0] = 0x43; // Operation code: READ TOC/PMA/ATIP
	cmd.CDBByte[1] = 0x00; // MSF=0, Format=0 (TOC)
	cmd.CDBByte[2] = 0x00; // Reserved
	cmd.CDBByte[3] = 0x00; // Reserved
	cmd.CDBByte[4] = 0x00; // Reserved
	cmd.CDBByte[5] = 0x00; // Reserved
	cmd.CDBByte[6] = 0x00; // Reserved
	cmd.CDBByte[7] = 0x00; // Allocation length MSB
	cmd.CDBByte[8] = 0x24; // Allocation length LSB (36 bytes, or 0x30/0xFF as needed)
	cmd.CDBByte[9] = 0x00; // Control
