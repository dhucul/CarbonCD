...
CDirStructure::CDirStructure(void)
{
    m_DirList = nullptr;
    m_FileList = nullptr;
    m_DirListTail = nullptr;
    m_FileListTail = nullptr;
    m_Next = nullptr;
    m_IsDirectory = false;
}
...
CDirStructure* CDirStructure::SearchName(LPCSTR Name)
{
    ...
    if (ret == nullptr && m_FileList != nullptr)
    {
...
{
    if (lstrcmp(Name, m_ImageFileName) == 0)
    {
        return this;
    }
...
{
    if (lstrcmp(Name, m_ImageFileNameShort) == 0)
    {
        return this;
    }
...
{
    if (lstrcmp(Name, m_RealFileName) == 0)
    {
        return this;
    }
...
void CDirStructure::CalcShortName(CWnd* ParentWnd)
{
    ...
    while (p != nullptr)
    {
        if (p->m_ImageFileNameShort.GetLength() > 8)
        {
            ...
            for (i = 0; i < 10; i++)
            {
                NewName.Format("%s~%d", p->m_ImageFileNameShort.Left(6), i);

                if (SearchNameShort(NewName) == nullptr)
                {
            ...
            if (i >= 10)
            {
                for (i = 0; i < 100; i++)
                {
                    NewName.Format("%s~%02d", p->m_ImageFileNameShort.Left(5), i);
            ...
            if (i >= 100)
            {
                for (i = 0; i < 10000; i++)
                {
                    NewName.Format("%s~%04d", p->m_ImageFileNameShort.Left(3), i);
    ...
    while (p != nullptr)
    {
        ...
        if (file.GetLength() > 12 || ext.GetLength() > 4)
        {
            ...
            for (i = 0; i < 10; i++)
            {
                NewName.Format("%s~%d%s", file.Left(6), i, ext.Left(4));
            ...
            if (i >= 10)
            {
                for (i = 0; i < 100; i++)
                {
                    NewName.Format("%s~%02d%s", file.Left(5), i, ext.Left(4));
            ...
            if (i >= 100)
            {
                for (i = 0; i < 10000; i++)
                {
                    NewName.Format("%s~%04d%s", file.Left(3), i, ext.Left(4));
        ...
        if (p->m_ImageFileName.GetLength() > 64)
        {
            ...
            {
                MessageBox(nullptr, cs, theSetting.m_Lang.m_Str[5], MB_OK | MB_TASKMODAL);
            }
        }
