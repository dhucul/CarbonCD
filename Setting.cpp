...
{
	// All supported Windows versions are NT-based, but for legacy code:
	// Use VersionHelpers.h instead of GetVersionEx
	return true;
}
...
void CSetting::Load(void)
{
	...
	m_ExtSetting_Write = ReadInt("WriteSetting", "ExtendedSettings", 0);
	{
		...
		// Replace deprecated GetVersionEx with VersionHelpers
		if (IsWindowsNT())
		{
			// Windows NT or later
			// You may want to check for a specific version if needed:
